...
def load_data(data_dir, max_word_length, fname_list, eos='+'):
    ...
    for fname in fname_list:
        print('number of tokens in %s: %d' % (fname, len(word_tokens[fname])))
    ...
    for fname in fname_list:
        ...
        word_tensors[fname] = np.array(word_tokens[fname], dtype=np.int64)
